...
async fn main() -> anyhow::Result<()> {
    ...
    // For other CLI commands that need direct DB access
    // IMPORTANT: If the daemon is running, LadybugDB is locked!
    if daemon_running {
    ...
    match command.as_str() {
        ...
        "ingest" => {
            ...
            let schema_str = if let Some(schema_path) = args.get(4) {
                ...
                r#"{ "languages": {} }"#.to_string() // Minimal fallback, should probably keep the old one but saving space
            };
