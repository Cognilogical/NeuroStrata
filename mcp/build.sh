...
echo "Building NeuroStrata Go MCP Server..."
...
for PLATFORM in "${PLATFORMS[@]}"; do
    ...
    OUTPUT_NAME="neurostrata-mcp-${OS}-${ARCH}"
    if [ "$OS" = "windows" ]; then
...
echo "Building NeuroStrata OpenCode Plugin..."
cd "$ROOT_DIR/plugins/opencode/neurostrata-plugin"
...
echo "Packaging NeuroStrata OpenCode Plugin..."
PLUGIN_TARBALL="$DIST_DIR/opencode-neurostrata.tgz"
npm pack --pack-destination "$DIST_DIR"
mv "$DIST_DIR"/opencode-neurostrata-*.tgz "$PLUGIN_TARBALL"
