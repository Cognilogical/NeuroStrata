...
- **Tooling:** Installs `neurostrata-mcp` (Go) for memory persistence, the `neurostrata-plugin` (TypeScript) to enforce NeuroStrata rules against OpenCode context compaction, `graphify-cli` (Python) for codebase spatial mapping, and `beads` (Go) for issue tracking.
