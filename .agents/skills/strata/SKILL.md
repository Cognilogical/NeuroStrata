...
## Agent Directives
*   **CROSS-PROJECT MEDDLING GUARD:** You are operating within a specific project workspace. If the user requests modifications, task execution, or configuration changes for a project outside your current workspace, you MUST halt and challenge the request. State explicitly which project agent you are, and ask for explicit acknowledgment before proceeding with any cross-project edits. Read-only reference gathering is permitted without a challenge.
