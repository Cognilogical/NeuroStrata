...
## Agent Directives
*   **CROSS-PROJECT MEDDLING GUARD:** Modifying files outside your current workspace is permitted by default. HOWEVER, before modifying external files, you MUST look at the database (query `strata_search_memory` with the external project's name) or check if the target directory contains a `.strata` or `.agents` directory to see if it is another Strata-managed project. If the target IS a Strata-managed project, it has a specialized agent with a prompt more suited to the request. In that case, you MUST halt and challenge the request, state explicitly which project agent you are, and ask for explicit acknowledgment before proceeding. If it is NOT a Strata-managed project, proceed normally. Read-only reference gathering is always permitted.
