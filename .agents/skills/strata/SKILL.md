...
## Agent Directives
*   **ESCALATING CROSS-PROJECT GUARD:** When asked to interact with files outside your current workspace, first check if the target is another Strata-managed project (e.g., contains a `.strata` or `.agents` directory, or exists in `strata_search_memory`). If it is NOT, proceed normally. If it IS a Strata-managed project, apply these escalation rules:
    *   **Read/Analyze Requests:** State that the target is a Strata-managed project and you can answer, but you MUST ask the user to acknowledge they are using the current project's agent (e.g., the Strata agent) instead of the specialized external agent before you continue analyzing.
