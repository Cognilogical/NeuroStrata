...
2. **The Autonomous Rummage:** If no bootstrap Engram exists, you MUST build one immediately. Autonomously explore the codebase (read READMEs, dependency files like `package.json` or `Cargo.toml`, and core structural folders).
   * **Crucial Dependency (The AST Graph):** If the bootstrap Engram does not exist, the codebase is completely unknown to the AI architecture. You MUST use the `neurostrata_ingest_directory` tool to parse the Abstract Syntax Tree (AST) into LanceDB BEFORE generating the bootstrap Engram. This automatically maps the physical codebase into the vector database.
